...
readonly POSTS_ROOT_DPATH="/documents/content/posts"
readonly INDEX_FNAME="index.md"

# コンテンツのルートディレクトリが存在することを確認
if [ ! -d ${POSTS_ROOT_DPATH} ]; then
  echo "" 1>&2
...
if [[ ! "${TARGET_DPATH}" == "${POSTS_ROOT_DPATH}/"* ]]; then
  echo "" 1>&2
...
# コマンド用ファイルパスを用意
readonly CMD_DPATH="posts${TARGET_DPATH/$POSTS_ROOT_DPATH/}"

# コマンド組み立て
readonly CMD="hugo new ${CMD_DPATH}/index.md"

echo "===== info ====="
echo "POSTS_ROOT_DPATH=${POSTS_ROOT_DPATH}"
echo "TARGET_DPATH=${TARGET_DPATH}"
echo "CMD_DPATH=${CMD_DPATH}"
echo "================"
echo "CREATE_FPATH=${TARGET_DPATH}/index.md"
echo "===== cmd  ====="
echo "${CMD}"
eval "${CMD}"
echo "... end."
